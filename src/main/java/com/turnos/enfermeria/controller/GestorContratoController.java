...
public class GestorContratoController {
    ...
    @Operation(summary = "Actualizar gestor de contrato",description = "Actualiza los datos de un gestor de contrato existente.",
            tags={"Contratos"})
    public ResponseEntity<GestorContratoDTO> ***REMOVED***(@RequestBody GestorContratoDTO gestorContratoDTO, @PathVariable Long idGestorContrato){
        return gestorContratoService.findById(idGestorContrato)
                .map(gestorContratoExistente -> ResponseEntity.ok(gestorContratoService.***REMOVED***(gestorContratoDTO, idGestorContrato)))
                .orElseThrow(() -> new GenericNotFoundException(
                        CodigoError.GESTOR_CONTRATO_NO_ENCONTRADO,
                        idGestorContrato,
                        request.getMethod(),
