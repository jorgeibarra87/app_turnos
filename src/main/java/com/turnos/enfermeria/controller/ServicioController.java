...
public class ServicioController {
    ...
    @Operation(summary = "Actualizar servicio", description = "Actualiza los datos de un servicio existente",
            tags={"Servicios", "Cuadro de Turnos"})
    public ResponseEntity<ServicioDTO> ***REMOVED***(@RequestBody ServicioDTO servicioDTO, @PathVariable("idServicio") Long ideServicio){
        return servicioService.findById(ideServicio)
                .map(servicioExistente -> ResponseEntity.ok(servicioService.***REMOVED***(servicioDTO, ideServicio)))
                .orElse(ResponseEntity.notFound().build());
    }
